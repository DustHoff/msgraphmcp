...
| `get_intune_app` | Get app details. Reads via `/beta` so Win32-only fields (`allowAvailableUninstall`, `displayVersion`, `installExperience.maxRunTimeInMinutes`) are returned; Win32-derived `select` tokens are auto-cast to `microsoft.graph.win32LobApp/<prop>`. | `appId`, `select` |
| `create_intune_web_app` | Add a web shortcut app | `displayName`, `publisher`, `appUrl` |
| `create_intune_store_app` | Add an iOS/Android store app. `windowsStore` maps to the legacy `windowsStoreApp` type, now rejected by Graph — use `create_intune_winget_app` for Windows. | `displayName`, `publisher`, `storeType` (`windowsStore`\|`iosStore`\|`androidStore`), `appStoreUrl` |
| `create_intune_winget_app` | Add a "Microsoft Store app (new)" via WinGet (`#microsoft.graph.winGetApp`). Replaces the deprecated `windowsStoreApp` type. | `displayName`, `publisher`, `packageIdentifier` (e.g. `9NBLGGH5R558`), `runAsAccount` (`system`\|`user`, default `system`), `description`, `informationUrl`, `privacyInformationUrl`, `isFeatured`, `roleScopeTagIds[]`, `manifestHash` |
| `update_intune_app` | Update app metadata (common mobileApp fields + Win32-specific). Win32 fields auto-mark the body as `#microsoft.graph.win32LobApp`. Beta-only Win32 fields (`allowAvailableUninstall`, `displayVersion`, `installExperience.maxRunTimeInMinutes`) auto-route the PATCH to `/beta` so the value persists (v1.0 silently drops them). | `appId`, `displayName`, `publisher`, `description`, `isFeatured`, `privacyInformationUrl`, `informationUrl`, `notes`, `owner`, `developer`, `roleScopeTagIds[]`, `largeIcon`, `installCommandLine`, `uninstallCommandLine`, `setupFilePath`, `applicableArchitectures`, `allowedArchitectures`, `minimumSupportedWindowsRelease`, `minimumFreeDiskSpaceInMB`, `minimumMemoryInMB`, `minimumNumberOfProcessors`, `minimumCpuSpeedInMHz`, `displayVersion`, `installExperience`, `returnCodes[]`, `rules[]`, `allowAvailableUninstall` |
| `delete_intune_app` | Delete an app | `appId` |
| `upload_win32_lob_app` | Upload a `.intunewin` Win32 LOB package. Source is **exactly one** of: (a) `filePath`, (b) `fileUrl`, or (c) a OneDrive reference (`oneDriveItemPath` **or** `oneDriveItemId`, optionally with `oneDriveUserId` — defaults to `"me"`). MSI packages auto-detect a ProductCode rule from `Detection.xml`; EXE packages must pass `rules`. | `filePath` \| `fileUrl` \| (`oneDriveItemPath` \| `oneDriveItemId` [+ `oneDriveUserId`]), `displayName`, `publisher`, `description`, `installCommandLine`, `uninstallCommandLine`, `setupFilePath`, `applicableArchitectures`, `minimumSupportedWindowsRelease`, `runAsAccount`, `deviceRestartBehavior`, `maxRunTimeInMinutes`, `displayVersion`, `rules[]`, `returnCodes[]`, `roleScopeTagIds[]` |
...
- **Detection rules at create time:** Intune API `2025-07-02` requires at least one detection rule on the `POST` — empty rule lists are rejected. For MSI packages the tool auto-injects a `#microsoft.graph.win32LobAppProductCodeRule` from the `<MsiInfo>` block in `Detection.xml` when `rules` is omitted. For EXE packages `rules` is required (`#microsoft.graph.win32LobAppFileSystemRule`, `…RegistryRule`, or `…PowerShellScriptRule`). After upload, assign to groups via `assign_intune_app`.
- **MSI metadata refresh on content updates:** `update_win32_lob_app_content` re-parses the new `.intunewin` and patches `msiInformation` and `fileName` along with `committedContentVersion`, so the Intune portal does not keep showing stale `productCode`/`productVersion` after a version bump.
